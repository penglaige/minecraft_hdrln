...
class DQNAgent(object):
    ...
    def __init__(self,env,q_func,optimizer_spec,num_actions,
        ...
        find_q = self.q_func(self.in_channels, skill_space[0]).type(dtype)
        find_q.load_state_dict(skill_pool[0])

        tool_q = self.q_func(1, skill_space[1]).type(dtype)
        tool_q.load_state_dict(skill_pool[1])

        self.skill_pool = [find_q, tool_q]  #skill network
        self.skill_action_space = ["primitive space", "find", "use"]

        self.freq = [0,0] # the frequency of using 3 skills
    ...
    def run(self, agent_host):
        ...
        for t in itertools.count():
            ...
            if (t > self.learning_starts and
                    self.hdrln_t % self.learning_freq == 0 and
